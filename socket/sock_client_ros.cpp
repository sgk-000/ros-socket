...
{
    joy_vel.linear.x = joy_msg.axes[1];
    joy_vel.linear.y = joy_msg.axes[0];
    joy_vel.angular.z = joy_msg.axes[2];
...
int main(void)
{
    ros::init(argc, argv, "socket_joy_publisher");
    ros::NodeHandle n;

    ros::publisher joy_pub = n.advertise<geometry_msgs::Twist>("joy_vel", 1000);
    ros::subscriber joy_sub = n.subscribe("joy", 10, joy_callback);
    ros::Rate loop_rate(10);

    while (ros::ok)
    {
        joy_pub.publish(joy_vel);
        ros::spinOnce();
        loop_rate.sleep();
    }
